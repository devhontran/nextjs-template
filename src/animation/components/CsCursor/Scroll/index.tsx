...
export default function CursorScroll(): React.ReactElement {
  ...
  useSignalEffect(() => {
    if (cursorType.value === CursorType.SCROLL) {
      motion({
        xPercent: 0,
      });
    } else {
  ...
  return (
    <Cursor className={styles.cursor_play}>
      <Box overflow={'clip'} ref={refCursor}>
        <VideoButton isIgnoreIcon>Keep scrolling</VideoButton>
      </Box>
    </Cursor>
  );
}
